...
# packer输出目录
ifeq ($(origin PACKER_OUT_DIR),undefined)
PACKER_OUT_DIR := $(ROOT_DIR)/output
endif
...
PACKER_VARS = PACKER_CACHE_DIR=$(PACKER_CACHE_DIR) \
	$(PACKER_LOG_ARG ) $(PACKER_FORCE_ARG) $(PACKER_DEBUG_ARG) \
	--var output_dir=$(PACKER_OUTPUT_DIR)

PACKER_VAR_FILES = -var-file=$(OS_VARS_DIR)/$(PACKER_OS_NAME)-$(PACKER_OS_VERSION)-$(PACKER_OS_ARCH).pkrvars.hcl
...
packer.build.template: 
	$(PACKER) build -only=global_image.$(PACKER_TEMPLATE_SOURCE).vm $(PACKER_VARS) $(PACKER_VAR_FILES) $(PACKER_TEMPLATE_DIR)

.PHONY: packer.build.custom
packer.build.custom: 
	$(PACKER) build -only=custom_image.$(PACKER_TEMPLATE_SOURCE).vm $(PACKER_VARS) $(PACKER_VAR_FILES)  $(PACKER_TEMPLATE_DIR)
